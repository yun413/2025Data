...
function askInRange(prompt, min, max) {
    let value;
    do {
        value = readline.questionInt(`${prompt} (${min}~${max}): `);
    }while (value<min || value>max);
    return value;
}
const weight = askInRange("Ur weight" , 10 , 120);
const height = askInRange("Ur height" , 60 , 230);
